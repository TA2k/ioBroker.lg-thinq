module.exports = {
    async createHeatRemoteStates(device, deviceModel) {
        ...
        const datapoints = ["airState.tempState.hotWaterTarget", "airState.opMode"];
        if (device.snapshot) {
            ...
            for (const remote of datapoints) {
                ...
                if (deviceModel.Value && deviceModel.Value[remote]) {
                    ...
                    if (valueObject) {
                        ...
                        await this.setStateAsync(device.deviceId + ".remote.basicCtrl." + laststate, device.snapshot[remote], true);
        ...
        await this.setStateAsync(`${device.deviceId}.remote.reservationCtrl.add_new_schedule`, false, true);
        common = {
            name: {
                "en": "Delete schedule",
        ...
        await this.setStateAsync(`${device.deviceId}.remote.reservationCtrl.del_new_schedule`, "00", true);
        await this.setObjectNotExistsAsync(
        ...
        await this.setStateAsync(device.deviceId + ".remote.reservationCtrl.send_new_schedule", false, true);
        ...
        if (
            ...
            if (on_data && on_data.length > 0) {
                for (const single in on_data) {
                    this.log.info(`Update: ${on_data[single]} - ${off_data[single]}`);
                    if (on_data.length > 0 && on_data[0].length === 7) {
    ...
    async createHeatSchedule(device, first, second) {
        ...
        await this.setStateAsync(`${device}.remote.reservationCtrl.${count}_state`, parseInt(active), true);
        common = {
            name: {
                "en": "switch on",
                "de": "einschalten",
        ...
        await this.setStateAsync(`${device}.remote.reservationCtrl.${count}_start`, hour + ":" + minute, true);
        common = {
            name: {
                "en": "switch off",
                "de": "ausschalten",
        ...
        await this.setStateAsync(`${device}.remote.reservationCtrl.${count}_end`, hour_off + ":" + minute_off, true);
    }
};
