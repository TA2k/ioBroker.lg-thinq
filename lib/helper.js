...
module.exports = {
    ...
    async createSendJson(devicedp) {
        ...
        await this.setObjectNotExistsAsync(`${devicedp}.remote.sendJSONNoSync`, {
            type: "state",
            common: {
                name: "sendJSONNoSync",
                type: "string",
    ...
    async createremote(devicedp, control, course, deviceType) {
        try {
            let states = {};
            let dev = "";
            ...
            if (control === "WMDownload") {
                await this.lastDeviceCourse(devicedp, deviceType);
                let common;
                common = {
    ...
    async lastDeviceCourse(devId, deviceType) {
        try {
            let devtype = await this.getStateAsync(`${devId}.deviceType`);
            if (!devtype || devtype.val == null) {
                this.log.debug(`Cannot found object ${devId}.deviceType`);
                if (deviceType) {
                    devtype = { val: deviceType };
                } else {
                    this.log.error(`Cannot found object ${devId}.deviceType`);
                    return;
                }
            }
            ...
            this.getForeignObject(id, async (err, obj) => {
                if (obj) {
                    ...
                    if (Array.isArray(rawstring) && Object.keys(rawstring).length > 0) {
                        ...
                        if (rawselect == null) {
                            this.log.warn(`Wrong State: ${JSON.stringify(rawstring)} -> ${states}`);
                            return;
                        }
                        this.log.debug(`${JSON.stringify(rawstring)} State: ${states}`);
                    ...
                    for (const key of fixDatapoints) {
                        if (
                            ...
                            if (this.remoteValue[`${element.deviceId}.${key}`] != element.data.state.reported[key]) {
                                ...
                                if (this.available401[`${element.deviceId}.remote.${folder}.${laststate}`]) {
