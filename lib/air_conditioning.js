                    ...
                    if (JSON.stringify(dataRemote).includes(remote)) {
                        if (dataRemote.dataKey) {
                                ...
                                if (valueObject.max) {
                                    ...
                                    values.forEach((value) => {
                                        ...
                                        if (typeof content === "string") {
                                            commons[value] = (langPack && langPack[content]) ? langPack[content].toString("utf-8") : content.replace("@", "");
                                        }
                                    });
                                }
                            }
                            ...
                            if (!obj) {
                                ...
                                const res = await this.setForeignObjectAsync(this.namespace + "." + device.deviceId + ".remote." + dataRemote.ctrlKey + "." + laststate, obj);
                            }
                ...
                case "holiday_silent_update":
                    ...
                    if (obj && obj.result && obj.result.data) {
                        ...
                        if (obj.result.data["airState.reservation.advancedSchedule"]) {
                            const lastValue = Object.keys(obj.result.data["airState.reservation.advancedSchedule"]).length;
                            for (const element of obj.result.data["airState.reservation.advancedSchedule"]) {
                                ++countValue;
                                if (countValue < lastValue) {
                                    next_type = obj.result.data["airState.reservation.advancedSchedule"][countValue].type;
                                } else {
                                ...
                                if (element.type != null && element.onOffFlag != null) {
                                    if (element.type === 2 && countValue != lastValue && first_type_2) {
                                        ...
                                        date_DP = element["startDate"].toString().length == 1 ? "0" + element.startDate : element.startDate + ".";
                                        date_DP += element["startMonth"].toString().length == 1 ? "0" + element.startMonth : element.startMonth + ".";
                                        date_DP += element.startYear + " ";
                                        date_DP += element["startHour"].toString().length == 1 ? "0" + element.startHour : element.startHour + ":";
                                        date_DP += element["startMin"].toString().length == 1 ? "0" + element.startMin : element.startMin;
                                        ...
                                        is_holiday_available = true;
                                        date_DP = element["endDate"].toString().length == 1 ? "0" + element.endDate : element.endDate + ".";
                                        date_DP += element["endMonth"].toString().length == 1 ? "0" + element.endMonth : element.endMonth + ".";
                                        date_DP += element.endYear + " ";
                                        date_DP += element["endHour"].toString().length == 1 ? "0" + element.endHour : element.endHour + ":";
                                        date_DP += element["endMin"].toString().length == 1 ? "0" + element.endMin : element.endMin;
                            ...
                            if (is_holiday_available === false || is_silent_available === false) {
                                ...
                                if (obj != null && obj.resultCode != null && obj.resultCode == "0000") {
                                    this.log.warn("Holiday/Silent Mode time maybe created. Please press holiday_silent_update again!");
                                } else {
