...
module.exports = {
                                ...
                                try {
                                    if (map === "min") {
                                        ...
                                        if (value === "moreLessTime") {
                                            ...
                                            common[map] = 360;
                                        } else if (this.modelInfos[devicedp] && this.modelInfos[devicedp]["signature"] &&
                                            (
    ...
    async sendStaticRequestThinq1(device, API1_CLIENT_ID) {
        ...
        } else if (period.val === 2) per = "Mon_";
        else if (period.val === 3) per = "Mon_";
        else if (period.val === 1) per = "Day_";
        else per = "Day_";
        ...
        try {
            ...
            this.getForeignObject(id, async (err, obj) => {
                if (obj) {
                    ...
                    const states = state != null && state.val !== true && state.val !== false && state.val != null ? state.val : "NOK";
        ...
        try {
            if (element.deviceId && this.modelInfos[element.deviceId]) {
                ...
                if (deviceType === 101 && platformType === "thinq2") {
                    ...
                    for (const key of fixDatapoints) {
                        if (
                            ...
                            {
                                this.remoteValue[`${element.deviceId}.snapshot.refState.${key}`]  = element.data.state.reported[key];
                                await this.setStateAsync(`${element.deviceId}.remote.${key}`, {
                ...
                if (deviceType === 406 && platformType === "thinq2") {
                    const fixDatapoints = ["airState.tempState.hotWaterTarget", "airState.opMode", "airState.operation"];
                    for (const key of fixDatapoints) {
                        if (
                            element.data != null &&
                            element.data.state != null &&
                ...
                if (deviceType === 401 && platformType === "thinq2") {
                    const fixDatapoints = ["airState.miscFuncState.hotWater", "airState.opMode", "airState.2nd.operation", "airState.miscFuncState.powerHotWater", "airState.2nd.tempState.target", "airState.quality.sensorMon", "airState.tempState.hotWaterTarget"];
                    for (const key of fixDatapoints) {
                        if (
                            element.data != null &&
                            element.data.state != null &&
