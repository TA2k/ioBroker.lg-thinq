module.exports = {
    async createHeatRemoteStates(device, deviceModel) {
        ...
        if (device.snapshot) {
            ...
            for (const remote of datapoints) {
                ...
                if (deviceModel.Value && deviceModel.Value[remote]) {
                    ...
                    const obj = await this.getObjectAsync(`${device.deviceId}.remote.basicCtrl.${laststate}`);
                    common = {
                        name: remote,
                        role: "state",
                        type: "mixed",
                    ...
                    if (valueObject) {
                        ...
                        if (!obj) {
                            await this.setObjectNotExistsAsync(`${device.deviceId}.remote.basicCtrl.${laststate}`, {
                                type: "state",
                                common: common,
                                native: {
                                    dataKey: remote,
                            ...
                            await this.setObjectAsync(`${device.deviceId}.remote.basicCtrl.${laststate}`, obj);
                        }
                        await this.setState(
                            `${device.deviceId}.remote.basicCtrl.${laststate}`,
                            device.snapshot[remote],
                            true,
                        );
            ...
            if (all_dp && all_dp.rows && on_data != null && all_dp.rows.length > length_data) {
                ...
                for (let i = start_point; i > 0; i--) {
                    ...
                    if (del_row > 0) {
                        ...
                        await this.delObjectAsync(`${device.deviceId}.remote.reservationCtrl.${state}_start`, {
                            recursive: true,
                        });
                        await this.delObjectAsync(`${device.deviceId}.remote.reservationCtrl.${state}_end`, {
                            recursive: true,
                        });
                        await this.delObjectAsync(`${device.deviceId}.remote.reservationCtrl.${state}_state`, {
                            recursive: true,
                        });
                    } else {
                        i = 0;
        ...
        await this.createDataPoint(
            `${device.deviceId}.remote.reservationCtrl.add_new_schedule`,
            common,
            "state",
            native,
        );
        ...
        await this.createDataPoint(
            `${device.deviceId}.remote.reservationCtrl.del_new_schedule`,
            common,
            "state",
            native,
        );
        await this.setState(`${device.deviceId}.remote.reservationCtrl.del_new_schedule`, "00", true);
        await this.setObjectNotExistsAsync(`${device.deviceId}.remote.reservationCtrl.send_new_schedule`, {
            type: "state",
            common: {
                name: {
                    en: "Send schedule",
    ...
    async addHeat(device) {
        const obj = await this.getObjectAsync(`${device}.remote.reservationCtrl.add_new_schedule`);
        ...
        const obj_del = await this.getObjectAsync(`${device}.remote.reservationCtrl.del_new_schedule`);
        if (obj_del && obj_del.common && obj_del.common.states) {
            const count = obj.native.data + 1;
            const new_count = count.toString().length === 1 ? `0${count}` : count;
            obj_del.common.states.push(new_count);
            this.extendObject(`${device}.remote.reservationCtrl.del_new_schedule`, {
                common: { states: obj_del.common.states },
            });
        }
    },
    ...
    async check_reservationCtrl(id, deviceId, lastsplit, state) {
        ...
        if (id.indexOf("_end") !== -1 || id.indexOf("_start") !== -1) {
            ...
            if (isfind != null) {
                ...
                for (let i = del_row; i > 0; i--) {
                    if (last_row === i) {
                        ...
                        await this.delObjectAsync(`${device}.remote.reservationCtrl.${del_last}_start`, {
                            recursive: true,
                        });
                        await this.delObjectAsync(`${device}.remote.reservationCtrl.${del_last}_end`, {
                            recursive: true,
                        });
                        await this.delObjectAsync(`${device}.remote.reservationCtrl.${del_last}_state`, {
                            recursive: true,
                        });
                        i = 0;
                    } else {
                ...
                const obj = await this.getObjectAsync(`${device}.remote.reservationCtrl.del_new_schedule`);
                if (obj && obj.common && obj.common.states != null) {
                    this.extendObject(`${device}.remote.reservationCtrl.del_new_schedule`, {
                        common: { states: obj.common.states.pop() },
                    });
                }
                const obj_add = await this.getObjectAsync(`${device}.remote.reservationCtrl.add_new_schedule`);
                if (obj_add && obj_add.native && obj_add.native.data != null) {
                    const new_dp = obj_add.native.data - 1;
                    this.extendObject(`${device}.remote.reservationCtrl.add_new_schedule`, {
                        native: { data: new_dp },
                    });
                }
            } else {
